...
test('checks DispatchAddBlog', () => {
    const action = DispatchAddBlog(blogs[0].title, blogs[0].content, blogs[0]._id)
...
test('checks DispatchRemoveBlog', () => {
    const action = DispatchRemoveBlog(blogs[0]._id)
    expect(action).toEqual({
        type: 'REMOVE_MY_BLOG',
        id: blogs[0]._id
    })
...
test('checks DispatchEditMyBlog', () => {
    const action = DispatchEditMyBlog(blogs[1]._id, blogs[1].title, blogs[1].content)
    expect(action).toEqual({
        type: 'EDIT_MY_BLOGS',
        id: blogs[1]._id, 
        title: blogs[1].title, 
